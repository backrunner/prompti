...
def main():
    ...
    for entry in manifest:
        ...
        before=(ROOT/f'output/imagegen/destinations/bold-v2/before/{key}.svg').read_text()
        cells.append(f'<article data-search="{html.escape(key + " " + entry["subject"] + " " + entry["subject_zh"])}"><b>{html.escape(key)}</b><span>{html.escape(entry["subject_zh"])}</span><div class="pair"><div class="old"><div class="art">{before}</div><small>旧版</small></div><div><div class="art new">{svg}</div><small>粗线简约版 · medium</small></div></div><div class="sizes"><div>{svg}<small>40px</small></div><div>{svg}<small>64px</small></div></div><small>{html.escape(entry["subject"])}</small></article>')
...
*{box-sizing:border-box}body{margin:0;padding:32px;background:var(--canvas);color:var(--text);font:15px -apple-system,BlinkMacSystemFont,sans-serif}h1{font-size:28px;margin:0 0 12px}p{color:var(--muted);line-height:1.6}nav{display:flex;gap:12px;flex-wrap:wrap;align-items:center;margin:24px 0}button,input{font:inherit;padding:10px 14px;border-radius:12px;border:1px solid var(--border);background:var(--surface);color:var(--text)}button{cursor:pointer}main{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:16px}article{text-align:center;background:var(--surface);border:1px solid var(--border);border-radius:20px;padding:20px}article[hidden]{display:none}.pair{display:flex;justify-content:center;gap:16px;margin-top:18px}.art{width:100px;height:100px;display:grid;place-items:center;background:var(--raised);border-radius:14px}.art svg{width:88px;height:88px}[fill="#000"]{fill:var(--ink)}b,span,small{display:block}b{font-size:15px;margin-bottom:6px}span{font-size:13px}small{font-size:11px;color:var(--muted);margin-top:8px}.sizes{display:flex;align-items:center;justify-content:center;gap:24px;min-height:110px}.sizes svg{width:40px;height:40px;padding:3.2px;background:var(--raised);border-radius:14px}.sizes>div:nth-child(2) svg{width:64px;height:64px;padding:5.12px}body.hide-old .old{display:none}body.hide-old .art{width:160px;height:160px}body.hide-old .art svg{width:144px;height:144px}@media(max-width:600px){body{padding:20px}main{grid-template-columns:1fr}}
</style><h1>65 个城市，粗线条与简约轮廓</h1><p>gpt-image-2.5-flare · medium · 单个主体、统一粗圆线条与少量识别特征。旧版为 2026-09-10 细线稿；40px / 64px 预览包含 App 同比例内边距。使用 Prompti 的翡翠绿 / 薄荷绿语义色。此页是资源审阅，非实际 App 截图；生成过程与实际验收见 <a href="README.md">README</a>。</p><nav><button id="theme" type="button" aria-pressed="false">深色预览</button><button id="compare" type="button" aria-pressed="true">隐藏旧版对照</button><input id="search" type="search" aria-label="搜索城市或地标" placeholder="搜索城市 ID 或地标名称"><span id="count">65 / 65</span></nav><main>'''+''.join(cells)+'''</main><script>
