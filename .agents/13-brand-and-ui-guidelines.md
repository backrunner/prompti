...
内置目的地不得再复用通用楼房、交通工具等 SF Symbols。每个稳定 ID 对应一幅独立的当地地标或知名代表物 SVG，图稿与资源清单见 [目的地图稿](../Documentation/Brand/DestinationArtwork/Gallery.html)。2026-09-17 起使用经审阅的 `gpt-image-2.5-flare` medium 粗线简约图标：单个主体、统一粗圆线条，以轮廓和少量特征表达地标，避免窗格、瓦片、网格与多主体构图。以 40pt 徽标（含组件内边距）检查可辨性，不以放大插画效果作为验收依据。生成图清理背景、统一留白后转为真实矢量路径；源 PNG、提示词、生成 ID 和处理脚本按 [目的地图稿说明](../Documentation/Brand/DestinationArtwork/README.md) 保留，不能只替换导出的 SVG，也不能把生成图称为手工原创或实际 App 截图。原生资源保留矢量并按语义色着色；自定义地点使用中性定位符，不虚构当地地标。目的地绘图源与 P 品牌标识源分别维护，不能互相替代。
...
| `promptDestructiveAction` | `#AE392F` | `#AE392F` | 原生左滑删除的实底；系统图标始终为白色，深色下也保持足够对比 |
...
- 连接验证使用共享 `PromptiConnectionTestRow`：状态与次级测试操作同一行，失败原因在下方，避免在设置中重复堆叠实底主按钮与成功段落；实际请求期间才出现系统加载指示，Reduce Motion 使用静态图标。
...
- 全部自有纵向滚动内容使用 `PromptiScrollView`；原生 `Form` 及需要系统左滑删除的复习 `List` 使用 `promptiScrollEdges()`。复习列表隐藏系统背景和分隔线，行内容继续使用 `promptiSurface()`。上方还有内容时才显示顶部渐隐，下方还能继续滚动时才显示底部渐隐；到达边界、内容不满一屏或筛选缩短列表时及时移除对应遮罩。遮罩不截获手势，保留导航、底部安全区和键盘适配。
...
- 仅在状态转换时使用短促淡入/位移；Logo 保持静止。不得增加装饰性的无限旋转、呼吸光效或与实际进度无关的伪百分比。真实异步请求等待期间使用系统 `ProgressView` 加载指示，结束后消失；Reduce Motion 使用静态状态图标或淡入形式。
