...
## 2026-09-17：场景目录与题目查重

- `localScenes` 保留原有 ID 用于历史题目标签解析，不再出现在新练习的选择或默认请求中。可选项保留餐饮、购物、交通等通用分类及用户自定义场景；食物、店铺、地标不单独成为选项。
- `DestinationCultureCatalog` 覆盖全部 65 个内置目的地、31 个国家和地区：每个地点均有当地生活背景，叠加地域文化、情境礼仪和当前学习语言的表达提示。目录示例不是封闭话题清单；不只围绕饮食或地标生成。学习语言不同于当地语言时采用该学习语言的自然礼貌表达，不混入另一语言的敬语词尾或强制俚语。新增目的地必须通过覆盖测试。
- `PromptBuilder.facts(for:)` 在请求时把原目的地事实与当前文化背景合并去重；生成、审核、本地 fact ID 校验和来源快照统一使用这份列表。只发送当前城市、地区与学习语言相关的背景（新增背景每请求少于 2000 UTF-8 字节），不发送整份目录。旧 Destination 编解码与题目快照不变，无数据库迁移。
- 自定义目的地通过英文/中文地区名及地区代码匹配已有地域背景，不冒用同名城市的事实；未知地区仍以普通礼貌交流为基础，不编造地方习俗。AI 在原生成请求内扩展具体情境，题目仍归属用户选中的父级 `sceneID`。
- 默认练习包含餐饮、交通；服务按批轮换选中的分类，并分配不同的沟通目标与探索角度。即使只选一个分类，也要求变化具体情境、对象和问题，而不只替换数字、人名或空位。
