...
- 在调用模型前按所选场景 ID 分配名额。完整新题组中，各场景题数最多相差 1；例如 10 题、3 个场景分配为 4/3/3。重复场景 ID 不增加权重，自定义场景与内置场景同等处理。
- 题量不足以覆盖全部场景或不能整除时，优先分给同目的地、同学习语言历史生成较少的场景；历史数量相同时随机打破平局。
- 每条生成及审核链只接收一个指定场景。并发完成顺序不改变名额，宽松模式的额外候选只能填补尚缺的场景；不通过修改题目标签来制造均衡。
- 会话重试按当前已保存题目计算缺口；提前备题按可用库存计算缺口。历史去重数据只影响平局优先级，不替代当前题组/库存计数。已有不均衡库存优先补少的场景，不删除已有题目。
- 如果某个场景在重试、预算或截止时间内始终无法通过审核，保留部分成功并允许显式重试，不让其他场景超额补位，也不降低审核标准。逐题交付期间和部分成功结果不保证瞬时均衡。
- 分配在本地完成，不增加规划模型调用，不改变 Provider 协议、持久化字段或练习统计口径。
