...
enum PromptBuilder {
    ...
    static func questionPrompt(_ request: TrainingRequest) throws -> String {
        ...
        - Within dining, explore different foods, drinks, desserts, shop formats and service situations that fit the destination. Within other scenes, similarly vary the setting, item, speaker role and practical problem. Stay inside the selected scene or custom topic; do not force food into unrelated scenes. For a custom destination without local facts, use ordinary fictional travel settings without inventing local specialties.
        - Choose a different combination of sub-setting, object and communication goal for each exercise, including when only one scene is selected. Mix selected types; vary register and turn direction. Avoid expressions and near-duplicate situations/intents in avoidRepeating: changing a name, quantity or blank position alone is not a new conversation. Return only the exercises, no planning output.
        """
    }
